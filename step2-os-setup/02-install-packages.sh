...
if ! bpftool version >/dev/null 2>&1; then
    echo "  bpftool 이 linux-tools 에 없음 → 소스 빌드 (ci/install-bpftool.sh)"
    bash "$(dirname "$0")/../ci/install-bpftool.sh"
fi
...
echo "[6/7] 시간 동기화 (chrony; 04-configure-ptp.sh 가 peer 모드로 설정)..."
...
echo -e "\n=========================================="
echo " 설치 완료. 검증: bash 01-check-prerequisites.sh"
echo " 다음: step3-kubernetes/01-prepare-node.sh (kubeadm/kubelet/kubectl/crictl)"
echo "=========================================="
