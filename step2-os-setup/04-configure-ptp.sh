...
#   - deploy-experiment.sh 는 이 파일을 호출하지 않는다 (run 은 chronyc tracking 을 meta.json 에 기록만 한다).
...
EOF
 ...
 deploy-experiment.sh run 은 sender 쪽 tracking 을 meta.json(chrony_before/after) 에 남긴다;
