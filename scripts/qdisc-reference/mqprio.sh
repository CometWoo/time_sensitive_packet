...
#   - scripts/experiment.sh 는 이 파일을 호출하지 않는다. 실험은 HTB + prio 를 쓴다 (ADR-0002).
...
실험은 단일 큐에서도 동작하는 HTB + prio 를 쓴다 (scripts/experiment.sh). 중단.
EOF
    exit 1
fi
...
==========================================
 ...
 다음: etf.sh (tc0 큐에 ETF)  — 단, SO_TXTIME 없는 talker 패킷은 ETF 가 버린다 (그 파일 헤더 참고)
==========================================
EOF
