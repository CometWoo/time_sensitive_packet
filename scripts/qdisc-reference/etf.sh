...
#   - scripts/experiment.sh 는 이 파일을 호출하지 않는다. 실험 경로에 ETF 는 없다 (ADR-0002).
...
tc qdisc show dev "$IFACE" | grep -q '^qdisc mqprio 100: root' \
    || { echo "mqprio 100: 이 없다 — mqprio.sh 먼저 (TX 큐 >= 3 필요)"; exit 1; }
...
cat <<'EOF'
  ...
  ⚠ 경고: 이 ETF 는 SO_TXTIME 으로 송신 시각을 지정한 패킷만 통과시킨다. 이 저장소의 talker.py 는
    ...
    학습/참고용으로만 붙이고, scripts/experiment.sh 를 돌리기 전에 반드시 제거하라:
...
==========================================
 ...
 다음: taprio.sh (802.1Qbv 게이트 스케줄; mqprio 를 통째로 대체한다)
==========================================
EOF
