...
#   - scripts/experiment.sh 는 이 파일을 호출하지 않는다. 실험의 qdisc 는 HTB 병목 + prio/… leaf 이며
#     scripts/experiment.sh 가 직접 만든다 (ADR-0002). 커밋된 결과에도 쓰이지 않았다.
...
# ──────────────────────────────────────────────────────────────────────────────
set -euo pipefail
# shellcheck source=scripts/qdisc-reference/common.sh
...
echo "=========================================="
[ "$TXQ" -ge 3 ] || { echo "TX 큐 $TXQ 개 < 3: mqprio/taprio num_tc 3 불가. 실험은 HTB + prio (scripts/experiment.sh). 중단."; exit 1; }
...
# ETF on tc0 (100:1 = TXQ0). ⚠ SO_TXTIME 없는 패킷(talker.py) 은 전량 드롭 — etf.sh 헤더 참고.
...
==========================================
 완료 ($MODE). ⚠ 이 상태로 scripts/experiment.sh 를 돌리지 마라 — ETF 가 TS 패킷을 버린다.
