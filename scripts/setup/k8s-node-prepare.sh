...
#     kubelet 설정은 k8s-init-control-plane.sh 의 kubeadm config(KubeletConfiguration) 로 들어간다.
# ──────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
if ! systemctl is-active --quiet containerd; then
    echo "  containerd 미실행 — scripts/setup/install-packages.sh 먼저"; exit 1
fi
grep -q 'SystemdCgroup = true' /etc/containerd/config.toml 2>/dev/null \
    || { echo "  SystemdCgroup=true 가 아님 — install-packages.sh 의 containerd 블록 참고"; exit 1; }
...
echo -e "\n=========================================="
echo " 노드 준비 완료"
echo "   control-plane: bash k8s-init-control-plane.sh"
echo "   worker:        sudo bash k8s-join-worker.sh \"<kubeadm join ...>\""
echo "=========================================="
