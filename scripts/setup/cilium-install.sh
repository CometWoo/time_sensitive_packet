...
#   - scripts/experiment.sh 는 이 파일을 호출하지 않는다 (run 은 cilium-config 의 routing-mode 와 이미지를
...
# 터널(vxlan) 모드면 NIC egress 에서 Pod IP/UDP 6000 이 캡슐화돼 u32/분류기가 보지 못한다 (scripts/experiment.sh 가 거부).
...
if [ "$(printf '%s\n' 6.6 "$KMM" | sort -V | head -1)" = 6.6 ]; then
    if command -v bpftool >/dev/null 2>&1; then
        ...
        echo "  [WARN] bpftool 없음 — tcx 확인 생략 (scripts/setup/install-packages.sh)"
    fi
else
...
==========================================
 Cilium $CILIUM_VERSION 설치 완료
   검증: bash cilium-verify.sh   /   cilium connectivity test (선택, ~5분)
   다음: bash scripts/experiment.sh build-ebpf && bash scripts/experiment.sh deploy-k8s
==========================================
EOF
