...
#     도 기본적으로 이 노드에만 건다.
#   - 사용법: sudo bash k8s-join-worker.sh "kubeadm join 192.168.x.x:6443 --token ... --discovery-token-ca-cert-hash sha256:..."
# ──────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
==========================================
 ...
 시간 동기화: sudo bash scripts/setup/ptp.sh worker <control-plane-ip>
==========================================
EOF
