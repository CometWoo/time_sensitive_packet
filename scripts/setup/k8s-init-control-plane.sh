...
#   - control-plane 노드가 실험의 sender(talker) 노드다: scripts/experiment.sh 는 여기서 sudo 로 실행하고
...
==========================================
 ...
 worker 에서:
   sudo $JOIN_CMD
 다음: scripts/setup/cilium-install.sh
 scripts/experiment.sh 를 sudo 로 돌릴 때 kubeconfig: KUBECONFIG=/etc/kubernetes/admin.conf (experiment.env)
==========================================
EOF
