...
#
# 사용: sudo bash run.sh [--runs N] [--conditions "a b c"] [--rate-mbps 20] [--flood-mbps 30]
#                                [--count 10000] [--interval-ms 1] [--out DIR] [--quick]
# =============================================================================
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$HERE/.." && pwd)"
BPF_BUILD="$ROOT/bpf/build"
LISTENER="$ROOT/workload/listener.py"
TALKER="$ROOT/workload/talker.py"
...
for f in "$BPF_BUILD/ts_classifier.bpf.o" "$BPF_BUILD/prio_probe.bpf.o"; do
    [ -f "$f" ] || { echo "$f 없음 — make -C bpf"; exit 1; }
done
mkdir -p "$OUT"
log() { echo "[$(date +%H:%M:%S)] $*"; }
...
run_one() {   # $1 = condition, $2 = run index
    ...
    ip netns exec $RECV_NS python3 "$ROOT/workload/udp_sink.py" --port $BE_PORT --stats-file "$dir/sink.json" &
    local sink_pid=$!
    ...
    ip netns exec $SEND_NS python3 "$ROOT/workload/be_flood.py" --target $RECV_IP --port $BE_PORT \
