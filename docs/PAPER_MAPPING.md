...
|---|---|---|
| Cilium 기반 K8s, underlay/overlay 혼합 | Cilium native routing, kube-proxy 대체 (`scripts/setup/`) | overlay 없음 → vef 의 우회 역할이 무의미 |
...
| **eg** (NIC egress: skb->priority → TC 클래스) | `bpf/src/ts_classifier.c` — NIC egress, tcx BEFORE Cilium | 논문의 eg 위치와 같다. 분류 기준에 UDP 포트 추가 ([ADR-0014](adr/0014-udp-port-classification-and-trust-boundary.md)), DSCP 마킹 추가 ([ADR-0007](adr/0007-dscp-marking-for-fabric-qos.md)) |
...
| ETF (CLOCK_TAI, Δ 150 μs, tc0) | 참고 스크립트만 (`scripts/qdisc-reference/etf.sh`) | talker 에 SO_TXTIME 없음 → 전량 드롭 위험 |
| ETS 게이트 (125/125/750 μs, 1 ms 주기) = 802.1Qbv | 참고 스크립트 (`taprio.sh`) | 리눅스 `taprio`. `sch_ets`(802.1Qaz) 와 이름 충돌 주의 |
...
| 실험: talker/listener 1 ms UDP, CPU 부하 경쟁 | `workload/`, stress DaemonSet(옵션) | 이 저장소는 **네트워크 경합**(BE 홍수 + 병목)을 추가 ([ADR-0006](adr/0006-testbed-needs-contention.md)) |
