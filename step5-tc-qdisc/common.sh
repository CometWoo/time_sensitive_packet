#!/bin/bash
# step5-tc-qdisc/common.sh — 참고 스크립트(mqprio / ETF / taprio) 공통 정의. 직접 실행하지 않는다.
# ──────────────────────────────────────────────────────────────────────────────
# STATUS: reference-only (2026-09)
#   - deploy-experiment.sh 는 step5 를 호출하지 않는다. 실험의 qdisc 는 HTB 병목 + {pfifo|fq_codel|
#     pfifo_fast|prio} leaf 이고 deploy-experiment.sh apply_condition() 이 직접 만든다 (ADR-0002).
