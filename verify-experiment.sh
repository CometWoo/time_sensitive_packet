#!/bin/bash
# =============================================================================
# verify-experiment.sh — 실험 상태 / 결과 검증 (deploy-experiment.sh 의 짝)
#
# 무엇을 검증하나
...
#
# 사용법
#   sudo bash verify-experiment.sh [--condition <cond>] [--results DIR] [--quick]
#   bash verify-experiment.sh --offline [--results DIR ...]      # CI: 커밋된 CSV 만 (root 불필요)
#
#   --condition  기대 상태를 고정한다: *_clsf 면 분류기가 붙어 있어야, 아니면 없어야 하고 leaf 가 맞아야 한다.
#                생략하면 현재 상태를 보고만 한다 (모순 — pin 은 있는데 체인 맨 앞이 아님 — 만 실패).
#   --offline    live 검사를 건너뛴다. DIR 을 주지 않으면 step8-measurement/results (2026-05, v1, meta 없음)
...
# 설정: ./experiment.env (deploy-experiment.sh 와 같은 파일/파서)
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TCX_ATTACH="$SCRIPT_DIR/step6-ebpf/build/tcx_attach"
...
# ── experiment.env (deploy-experiment.sh 와 동일 규칙) ─────────────────────────
...
echo "================================================================"
echo "  verify-experiment — $(date -u +%FT%TZ)  mode=$([ "$OFFLINE" = 1 ] && echo offline || echo live)${CONDITION:+  condition=$CONDITION}"
echo "================================================================"

kernel_ge() { local have; have=$(uname -r | cut -d. -f1-2); [ "$(printf '%s\n' "$1" "$have" | sort -V | head -1)" = "$1" ]; }

# kubeconfig (deploy-experiment.sh 와 같은 탐색, 실패해도 계속 — 클러스터 검사만 건너뛴다)
...
live_checks() {
    ...
    if [ "$mode" = tcx ] && [ ! -x "$TCX_ATTACH" ]; then
        warn "$TCX_ATTACH 없음 — kernel >= 6.6 에서는 tcx 로 붙어야 한다 (bash deploy-experiment.sh build-ebpf)"
    fi
    ...
    if [ "$mode" = tcx ] && [ -x "$TCX_ATTACH" ]; then
        ...
        if [ "$attached" = 1 ]; then
            ...
            [ -e "$PIN_LINK" ] || warn "링크 pin $PIN_LINK 없음 — deploy-experiment.sh 가 만든 링크가 아니다 (detach 는 pin 으로 한다)"
...
results_checks() {
    ...
    if [ "${#dirs[@]}" -eq 0 ]; then
        if [ "$OFFLINE" = 1 ]; then
            dirs=("$SCRIPT_DIR/step8-measurement/results" "$SCRIPT_DIR/results")
        else
            dirs=("$RESULTS_DIR")
        fi
    fi
...
for path in files:
    ...
    legacy_v1 = "step8-measurement/results/" in path.replace("\\", "/")   # 2026-05 v1 데이터 (meta 이전)
    ...
    if meta is None:
        ...
        continue
    # ── meta 기반 검사 ──
    # k8s meta 는 "classifier": bool 을 쓰고, testbed(run_testbed.sh) meta 는 조건 이름(*_clsf) 으로만 안다
