#!/bin/bash
# =============================================================================
# hubble-monitor.sh — Cilium Hubble 로 실험 트래픽(UDP TS_PORT) 흐름 관찰 (선택 도구)
#
# 상태:
#   step4-cilium/01-install-cilium.sh 의 helm 설치에는 hubble 옵션이 없다 → 기본 **비활성**.
#   먼저 'enable' 로 켠다 (helm upgrade --reuse-values --version <설치된 차트 버전>; Cilium 버전은 그대로,
#   agent 만 재시작된다 — 재시작 뒤에도 우리 tcx 링크(pin)는 남지만, deploy-experiment.sh run 이 체인
#   순서를 다시 확인한다). 재시작 전후로 meta.json 의 cilium_image 가 같아야 한다.
#   deploy-experiment.sh cleanup 이 'stop' 을 호출해 백그라운드 캡처를 끝낸다.
#
# 한계:
...
#   listener CSV + ts_counters + tc -s class (deploy-experiment.sh / verify-experiment.sh) 로 본다.
#
# 사용법:
#   bash hubble-monitor.sh enable      # Hubble + relay + UI 활성화 (1회)
#   bash hubble-monitor.sh status      # Hubble 상태
#   bash hubble-monitor.sh watch       # 실험 트래픽 실시간 관찰
#   bash hubble-monitor.sh start <out> # 백그라운드 캡처 시작 → <out> 파일 (JSON)
#   bash hubble-monitor.sh stop        # 백그라운드 캡처 종료 (없어도 성공)
...
stop_capture() {   # 멱등: 캡처가 없어도 0 으로 끝난다 (deploy-experiment.sh cleanup 이 호출)
    if [ -f "$PIDFILE" ]; then
